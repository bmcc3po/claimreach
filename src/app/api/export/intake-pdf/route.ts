...
function answerText(field: any, leadRow: any, answers: Record<string, any>): string {
  ...
  // writes there, keyed by field id, regardless of a field's declared scope. Read
  // there FIRST; fall back to a real leads column only when the jsonb is empty.
  // (Previously keyed lead-scope fields off the leads table only, so every
  // console-captured answer exported as "—".)
