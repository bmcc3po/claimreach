...
export async function ingestComm(c: {
  ...
  const admin = supabaseAdmin();
  // de-dupe
  if (c.call_sid) { const { data } = await admin.from("communications").select("id, lead_id").eq("call_sid", c.call_sid).maybeSingle(); if (data) return await update(admin, data.id, c); }
  if (c.sms_sid) { const { data } = await admin.from("communications").select("id, lead_id").eq("sms_sid", c.sms_sid).maybeSingle(); if (data) return await update(admin, data.id, c); }
...
async function update(admin: any, id: string, c: any) {
  const patch: any = {};
